...
const areas = [
  { icon: Brain, title: 'Artificial Intelligence', desc: 'Predictive models and operational intelligence.', href: '/platform/innovation', color: '#C9782B' },
  { icon: Microscope, title: 'Research', desc: 'Agricultural technology research and development.', href: '/platform/innovation', color: '#4F9D5C' },
  { icon: Lightbulb, title: 'Open Innovation', desc: 'Collaborative development with partners.', href: '/platform/roadmap', color: '#7AC142' },
  { icon: Rocket, title: 'Future Infrastructure', desc: 'Next-generation capabilities and roadmap.', href: '/platform/roadmap', color: '#F99D1C' },
];
...
export default function InnovationOverview() {
  ...
  return (
    <Layout>
      <Seo title="Research & Innovation" description="Where AI, research, and open collaboration turn into the next capability on the platform." />
            ...
            {areas.map((a, i) => (
              <motion.div key={a.href} initial={{ opacity: 0, y: 20 }} animate={isVisible ? { opacity: 1, y: 0 } : {}} transition={{ delay: i * 0.1 }}>
                <Link to={a.href} className="group card-elevate block bg-surface rounded-lg border border-neutral-100 p-6 h-full text-center">
                  ...
                  <ArrowRight className="w-4 h-4 text-neutral-400 group-hover:text-brand-accent group-hover:translate-x-1 transition-all mx-auto" />
                </Link>
              </motion.div>
            ))}
          </div>
